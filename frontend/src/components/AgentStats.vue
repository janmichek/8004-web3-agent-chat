        ...
        <template v-else>
          ...
          <template v-else-if="memory">
            <div v-if="memory.empty" class="empty">
              ...
              <p class="hint">
                This agent has no <code>memory.json</code> history. Chat to create the first
                checkpoint — it persists to <code>agents/{{ agent.name }}/memory.json</code>.
              </p>
            ...
            <template v-else>
              <div class="grid">
                <div class="stat">
                  ...
                  <span class="sub mono"
                    >{{ stats?.humanCount }} you · {{ stats?.aiCount }} agent ·
                    {{ stats?.toolCount }} tool</span
                  >
                </div>
                <div class="stat">
                  ...
                  <span class="sub mono"
                    >{{ memory.checkpointCount }} checkpoint{{
                      memory.checkpointCount !== 1 ? "s" : ""
                    }}</span
                  >
                </div>
                <div class="stat">
                  ...
                  <span v-if="memory.lastActive" class="sub mono" :title="memory.lastActive">{{
                    new Date(memory.lastActive).toLocaleString()
                  }}</span>
                </div>
                <div class="stat">
                  <span class="k">ETH moved</span>
                  <span class="v mono">{{
                    stats && Number(stats.totalEthSent) > 0
                      ? Number(stats.totalEthSent)
                          .toFixed(8)
                          .replace(/0+$/, "")
                          .replace(/\.$/, "") + " ETH"
                      : "—"
              ...
              <div v-if="topTools.length" class="chips">
                <span
                  v-for="[name, n] in topTools"
                  :key="name"
                  class="chip mono"
                  :title="`${name} called ${n} times`"
                >
              ...
              <div class="lists">
                <div v-if="stats?.uniqueRecipients.length" class="list-block">
                  <span class="list-label"
                    >Recipients ({{ stats?.uniqueRecipients.length ?? 0 }})</span
                  >
                  <div class="addr-list">
                    <a
                      ...
                      >{{ shortAddr(a) }} ↗</a
                    >
                    <span v-if="(stats.uniqueRecipients.length ?? 0) > 6" class="mono more"
                      >+{{ (stats.uniqueRecipients.length ?? 0) - 6 }} more</span
                    >
                  </div>
                </div>
                ...
                <div v-if="memory.recentTxHashes.length" class="list-block">
                  <span class="list-label"
                    >Recent txs ({{ stats?.txHashes.length ?? memory.recentTxHashes.length }})</span
                  >
                  <div class="addr-list">
                    <a
