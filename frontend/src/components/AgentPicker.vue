...
const masterScanUrl = computed(() => {
  ...
  const base =
    masterChainId.value === 42161 ? "https://arbiscan.io" : "https://sepolia.arbiscan.io";
...
function friendlyError(err: unknown): string {
  ...
  if (/insufficient funds|insufficient balance/i.test(line))
    return "Master wallet has insufficient ETH.";
...
async function loadAgents() {
  ...
  try {
    ...
    loadError.value = err instanceof Error ? err.message : "API unavailable — run npm run serve";
  }
}
    ...
    <div class="sub-section">
      ...
      <div v-show="open" class="collapsible-body">
          ...
          <dl>
            ...
            <div v-if="agent.walletAddress">
              ...
              <dd class="mono">
                ...
                >
                <span v-else :title="agent.walletAddress">{{
            ...
            <div v-if="agent.agentId">
              <dt>Rating</dt>
              <dd class="mono" data-testid="agent-rating">
                {{ ratingLoading ? "…" : ratingText || "—" }}
              </dd>
            </div>
            <div>
              <dt>Tools</dt>
        ...
        <div v-if="agent" class="danger-zone">
          ...
          <p v-if="deleteStatusText" class="status" data-testid="delete-status">
            {{ deleteStatusText }}
          </p>
        </div>
          ...
          <template v-else>
            <form class="form" @submit.prevent="fund">
              ...
              <button
                ...
                :disabled="!canSend"
              >
                {{ fundBusy ? "Sending…" : "Send" }}
              </button>
            </form>
            <p
              v-if="fundStatusText"
              class="status"
              data-testid="fund-status"
              :class="fundStatusKind"
            >
              {{ fundStatusText }}
            </p>
          </template>
...
.btn.danger {
  color: #ffb4b0;
  border-color: color-mix(in oklab, #ffb4b0 40%, var(--border));
}
.master dt {
  font-size: 0.7rem;
  text-transform: uppercase;
