...
function normalizeConfig(parsed: Record<string, unknown>): AgentConfig {
  ...
  if (
    ...
    (parsed as unknown as AgentConfig).metadata?.standaloneTools &&
    !(parsed as unknown as AgentConfig).metadata?.tools
  ) {
...
export function saveAgentConfig(agentName: string, config: AgentConfig): void {
  ...
  try {
    ...
    if (process.env.VERCEL) {
      console.warn(
        `[config] Vercel write failed, set ${getAgentConfigEnvName(agentName)} instead:`,
        err,
      );
    } else {
      throw err;
